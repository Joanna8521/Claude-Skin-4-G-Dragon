...
<img src="docs/demo-cat.jpg" width="180" alt="示範照片">

*開工前，有些話想對你說*

**今天只要把 Python 2026 推上 GitHub 就贏了！**
...
在 `Python 2026` 開對話就顯示「今天也要把 Python 2026 好好推進一點喔！」，在 `Grow4ai` 就顯示 Grow4ai。**一句設定，每個專案都貼題。**
...
程式碼是 MIT，見 [LICENSE](LICENSE)。

**MIT 只涵蓋這個工具本身，不包含你餵給它的圖片。** 用 `claude-skin photo` 加的照片只存在你自己的電腦裡，永遠不會被 commit（已列入 gitignore）。那些圖片的權利歸屬是你和著作權人之間的事，本專案不授予你任何權利。

如果你 fork 這個 repo 並想在裡面附示範圖，請確定你**真的擁有散布該圖的權利**。網路上撿到的圖通常不算，就算它被轉貼到爛，就算它看起來像 AI 生成的，都一樣。
